...
def getBoundingBoxes(
    ...
    os.chdir(directory)
    files = glob.glob("*.txt")
    files = [f for f in files if not "step" in f]
    files.sort()
    ...
    for f in files:
        ...
        for line in fh1:
            ...
            if isGT:
                ...
                tokens = iter(splitLine[1:])
                coord = np.array(take(tokens, 3))
...
allBoundingBoxes, allClasses = getBoundingBoxes(
    gtFolder, True, gtFormat, gtCoordType, limit=args.limit, imgSize=imgSize
)
# Get detected boxes
allBoundingBoxes, allClasses = getBoundingBoxes(
    detFolder,
